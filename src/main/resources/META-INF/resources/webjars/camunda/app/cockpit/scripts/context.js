export default {
    ...
    render: async (node, {api, processInstanceId}) => {
        ...
        //without special context
            ...
            node.innerHTML = 'No context provided for this activity.'
            //default context can be used for testing
            /*
            node.innerHTML = `
                  <div>
    ...
    for (const variables_raw of variables_response) {
        if (!variables_raw.hasOwnProperty(0)) {}
            ...
            for (const variable_raw of variables_raw) {
                variables[variable_raw.name] = variable_raw.value; //processes/details are sorted after time, therefore the newest variable is chosen
            }
        }
    }
    return variables;
...
async function getActivityVariables(activityInstanceId) {
    ...
    for (const variables_raw of variables_response) {
        if (!variables_raw.hasOwnProperty(0)) {}
        else if (variables_raw[0].activityInstanceId === activityInstanceId) {
            for (const variable_raw of variables_raw) {
...
async function getActivityName(activityId)  {
    let activities = await (await fetch("http://localhost:8080/running/activities")).json();
    for (const activity of activities) {
